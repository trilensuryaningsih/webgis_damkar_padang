...
| H6 | Fase 5: CRUD + Admin page selesai | Manajemen data berfungsi |
| H7 | Fase 6: Testing + bug fix + polish | Siap presentasi ✅ |

---

---

## BAGIAN 3 — PEMBAGIAN TUGAS TIM

> Pembagian ini dirancang agar setiap anggota memiliki **beban kerja setara**, tanggung jawab yang **jelas batasnya**, dan **tidak saling blocking** satu sama lain.

---

## Peta Dependensi (Baca Ini Dulu!)

```
DIMAS push db.js + app.js + blankspot + stats
    ↓
RAFI + ZAKKY mulai backend (paralel, file berbeda)
    ↓
RAFI + ZAKKY selesai → TRILEN + ACID mulai frontend (paralel)
    ↓
Semua selesai → DIMAS koordinir testing & integrasi
```

**Aturan utama:**
- Jangan mulai bagian berikutnya sebelum bagian yang jadi **prasyarat** selesai
- Kalau ada file yang dipakai bersama (misal `api.js`), **ZAKKY yang pegang** dan share ke grup
- Gunakan **1 repo GitHub bersama** — setiap orang kerja di branch sendiri lalu pull request

---

## 👤 DIMAS — Backend Foundation + Analisis Spasial

**Role:** Project Lead & Backend Foundation  
**Estimasi:** H1–H2

### Fungsionalitas yang Diimplementasikan

| Kode | Fitur | Lewat File |
|------|-------|------------|
| F4 | Melihat Area Blank Spot (polygon merah di peta) | `blankspot.js` |
| F5 | Radius layanan dinamis — bagian backend recalculate | `blankspot.js?radius=` |
| F8 | Data untuk kartu statistik (luas kota, terlayani, blankspot, persen) | `stats.js` |

> Dimas menyediakan **data mentah** untuk F4, F5, F8. Tampilannya di peta dikerjakan Trilen (F4, F5) dan Acid (F8).

### Tanggung Jawab File

```
backend/src/
├── app.js            ← entry point Express, daftarkan semua route
├── config/
│   └── db.js         ← koneksi PostgreSQL pool
└── routes/
    ├── blankspot.js  ← GET /api/blankspot (+ radius dinamis)
    └── stats.js      ← GET /api/stats

frontend/src/
└── main.jsx          ← fix Leaflet default icon bug
```

### Task List

```
[ ] Buat db.js → koneksi PostgreSQL pool, log sukses/gagal
[ ] Buat app.js → entry point Express, pasang CORS, daftarkan
    semua route milik tim (damkar, coverage, blankspot, dll)
[ ] Buat .env dengan kredensial DB → share ke grup via chat
[ ] Buat route blankspot.js
    → GET /api/blankspot (pakai tabel blank_spot yang sudah ada)
    → GET /api/blankspot?radius=5000 (hitung ulang on-the-fly)
[ ] Buat route stats.js
    → GET /api/stats → return luas_kota, luas_terlayani,
      luas_blankspot, persen_blankspot, jumlah_pos
[ ] Edit main.jsx → tambahkan fix Leaflet default icon
[ ] Test endpoint miliknya:
    http://localhost:5000/api/blankspot
    http://localhost:5000/api/stats
[ ] Push ke GitHub → share link repo ke semua anggota
```

### Output yang Harus Diserahkan ke Tim

- ✅ `app.js` sudah mendaftarkan semua route (siap dipakai Rafi & Zakky)
- ✅ `db.js` bisa diimport dan koneksi DB berhasil
- ✅ File `.env` di-share via grup (jangan push ke GitHub)
- ✅ `GET /api/blankspot` → return GeoJSON blank spot
- ✅ `GET /api/stats` → return JSON statistik angka

---

## 👤 RAFI — Backend Spatial API

**Role:** Spatial Backend Developer  
**Estimasi:** H2

> **Prasyarat:** Tunggu Dimas push `db.js` dan `app.js` ke GitHub dulu, baru clone dan mulai.

### Fungsionalitas yang Diimplementasikan

| Kode | Fitur | Lewat File |
|------|-------|------------|
| F3 | Data area jangkauan (coverage) tiap pos | `coverage.js` |
| F5 | Radius layanan dinamis — bagian backend recalculate coverage | `coverage.js?radius=` |
| F6 | Data jaringan jalan Kota Padang | `jalan.js` |
| F9 | Data 5 titik kandidat rekomendasi lokasi baru | `rekomendasi.js` |
| F10 | Data list rekomendasi untuk sidebar | `rekomendasi.js` |

> Rafi menyediakan **data mentah** untuk F3, F5, F6, F9, F10. Tampilannya di peta dikerjakan Trilen, tampilannya di sidebar dikerjakan Acid.

### Tanggung Jawab File

```
backend/src/routes/
├── coverage.js      ← GET /api/coverage (buffer 3km + radius custom)
├── rekomendasi.js   ← GET /api/rekomendasi (5 kandidat GeoJSON)
└── jalan.js         ← GET /api/jalan (jaringan jalan GeoJSON)
```

### Task List

```
[ ] Clone repo dari Dimas
[ ] Buat route coverage.js
    → Query ST_Buffer dari data_damkar_padang
    → Support query param ?radius=xxxx
[ ] Buat route rekomendasi.js
    → Query dari tabel kandidat_rekomendasi
    → Return GeoJSON FeatureCollection
[ ] Buat route jalan.js
    → Query dari tabel jaringan_jalan
    → Filter hanya jalan utama (primary, secondary, tertiary, dll)
[ ] Test semua endpoint di browser:
    http://localhost:5000/api/coverage
    http://localhost:5000/api/rekomendasi
    http://localhost:5000/api/jalan
[ ] Cek: semua return GeoJSON yang valid (buka di geojson.io)
[ ] Push ke branch → pull request ke main
```

### Output yang Harus Diserahkan ke Tim

- ✅ `GET /api/coverage` → GeoJSON 7 polygon buffer
- ✅ `GET /api/rekomendasi` → GeoJSON 5 titik kandidat
- ✅ `GET /api/jalan` → GeoJSON polyline jalan

---

## 👤 ZAKKY — Backend CRUD + Frontend Bridge

**Role:** CRUD Developer & API Integration  
**Estimasi:** H2 backend + awal H3 api.js

> **Prasyarat:** Tunggu Dimas push `db.js` dan `app.js` ke GitHub dulu.

### Fungsionalitas yang Diimplementasikan

| Kode | Fitur | Lewat File |
|------|-------|------------|
| F2 | Data 7 pos damkar (GeoJSON untuk peta) | `damkar.js` GET `/api/damkar` |
| F11 | CRUD pos damkar — backend (tambah, edit, hapus) | `damkar.js` POST/PUT/DELETE |
| F12 | Searching pos damkar — backend + semua fungsi API | `damkar.js` GET `/list?search=` + `api.js` |

> Zakky menyediakan **data + semua fungsi axios** yang dipakai seluruh tim frontend (Trilen & Acid). Tanpa `api.js` dari Zakky, frontend tidak bisa fetch data apapun.

### Tanggung Jawab File

```
backend/src/routes/
└── damkar.js        ← SEMUA endpoint damkar (GET/POST/PUT/DELETE + search)

frontend/src/
└── services/
    └── api.js       ← Semua fungsi axios (dipakai Trilen & Acid)
```
...
BACKEND
[ ] Clone repo dari Dimas
[ ] Buat damkar.js dengan 6 endpoint:
    → GET /api/damkar          (GeoJSON untuk peta)
    → GET /api/damkar/list     (tabel, support ?search=)
    → GET /api/damkar/:id      (detail 1 pos)
    → POST /api/damkar         (tambah pos baru)
    → PUT /api/damkar/:id      (edit pos)
    → DELETE /api/damkar/:id   (hapus pos)
[ ] Test CRUD pakai Postman atau Thunder Client (VS Code extension)
    - POST: kirim body JSON → cek data masuk ke PostgreSQL
    - PUT: ubah nama → cek berubah di DB
    - DELETE: hapus → cek hilang dari DB
    - GET /list?search=koto → cek filter berfungsi
[ ] Push backend ke branch → pull request

FRONTEND
[ ] Buat services/api.js dengan semua fungsi:
    - getDamkar, getCoverage, getBlankspot
    - getRekomendasi, getJalan, getStats
    - getDamkarList, getDamkarById
    - createDamkar, updateDamkar, deleteDamkar
[ ] Share ke grup: "api.js sudah siap, silakan import"
[ ] Push frontend ke branch → pull request
```

### Output yang Harus Diserahkan ke Tim

- ✅ Semua 6 endpoint `/api/damkar` berfungsi dan sudah ditest Postman
- ✅ `api.js` sudah di-push dan bisa langsung di-import Trilen & Acid
- ✅ Dokumentasi singkat di grup: nama fungsi dan parameternya

---

## 👤 TRILEN — Frontend Peta (Map Components)

**Role:** Map UI Developer  
**Estimasi:** H3–H4

> **Prasyarat:** Tunggu Dimas push `main.jsx` + Zakky push `api.js`.

### Fungsionalitas yang Diimplementasikan

| Kode | Fitur | Lewat File |
|------|-------|------------|
| F1 | Peta interaktif Kota Padang (zoom, pan, tile OSM) | `MapContainer.jsx` |
| F2 | 7 marker merah pos damkar + popup (nama, no_pos) | `DamkarMarkers.jsx` |
| F3 | Layer hijau area jangkauan (coverage) tiap pos | `CoverageLayer.jsx` |
| F4 | Layer merah area blank spot di peta | `BlankSpotLayer.jsx` |
| F5 | Update visual coverage & blank spot saat slider radius bergeser | `CoverageLayer.jsx` + `BlankSpotLayer.jsx` |
| F6 | Layer polyline jaringan jalan Kota Padang | `JalanLayer.jsx` |
| F9 | 5 marker hijau kandidat rekomendasi + popup skor | `RekomendasiMarkers.jsx` |

> Trilen mengerjakan **semua yang tampil di atas peta**. Sidebar di sebelah kirinya dikerjakan Acid. Keduanya disatukan di `MapPage.jsx`.
...
├── components/
│   └── Map/
│       ├── MapContainer.jsx       ← Peta utama Leaflet
│       ├── DamkarMarkers.jsx      ← 7 marker merah pos damkar
│       ├── CoverageLayer.jsx      ← Layer hijau coverage
│       ├── BlankSpotLayer.jsx     ← Layer merah blank spot
│       ├── JalanLayer.jsx         ← Layer polyline jalan
│       └── RekomendasiMarkers.jsx ← 5 marker hijau rekomendasi
└── pages/
    └── MapPage.jsx                ← Halaman peta (gabungkan Map + Sidebar)
```

### Task List

```
[ ] Pull repo terbaru (setelah Dimas + Zakky push)
[ ] Buat MapContainer.jsx
    → Koordinat center Padang: [-0.9492, 100.3543]
    → Zoom level 12
    → TileLayer OpenStreetMap
    → Terima props: layers (object), radius (number)
[ ] Buat DamkarMarkers.jsx
    → Fetch dari getDamkar()
    → Custom icon merah
    → Popup: nama_lokasi, no_pos
[ ] Buat CoverageLayer.jsx
    → Fetch dari getCoverage(radius)
    → Re-render saat radius berubah (pakai key={radius})
    → Warna: hijau transparan
[ ] Buat BlankSpotLayer.jsx
    → Fetch dari getBlankspot(radius)
    → Re-render saat radius berubah
    → Warna: merah transparan
[ ] Buat JalanLayer.jsx
    → Fetch dari getJalan()
    → Warna: abu-abu, opacity 0.6
[ ] Buat RekomendasiMarkers.jsx
    → Fetch dari getRekomendasi()
    → Custom icon hijau
    → Popup: nama, luas_km2, skor_prioritas
[ ] Buat MapPage.jsx
    → Gabungkan MapContainer + Sidebar (dari Acid)
    → State: layers (object toggle) + radius (number)
[ ] Test: semua layer tampil di peta, popup berfungsi
[ ] Push ke branch → pull request
```

### Output yang Harus Diserahkan ke Tim

- ✅ Peta Padang muncul di browser
- ✅ 7 marker damkar tampil + popup berfungsi
- ✅ Layer coverage, blankspot, jalan, rekomendasi tampil
- ✅ `MapPage.jsx` siap di-import ke `App.jsx` milik Acid

---

## 👤 ACID (Arrasyid) — Frontend UI Sidebar + Halaman Admin

**Role:** UI Developer & Admin Page  
**Estimasi:** H3–H5

> **Prasyarat:** Tunggu Dimas push `main.jsx` + Zakky push `api.js`.  
> Untuk `MapPage.jsx` di dalam `App.jsx`, koordinasikan dengan Trilen.

### Fungsionalitas yang Diimplementasikan

| Kode | Fitur | Lewat File |
|------|-------|------------|
| F5 | Slider radius di sidebar (kirim nilai ke MapPage → Trilen render ulang) | `LayerControl.jsx` |
| F7 | Checkbox toggle on/off tiap layer peta | `LayerControl.jsx` |
| F8 | Kartu statistik blank spot (luas kota, terlayani, blankspot, persen) | `StatsPanel.jsx` |
| F10 | List 5 kandidat rekomendasi di sidebar | `RekomendasiPanel.jsx` |
| F11 | UI CRUD — form tambah pos, form edit, tombol hapus + konfirmasi | `DamkarForm.jsx` + `DamkarTable.jsx` + `AdminPage.jsx` |
| F12 | UI Searching — input search di tabel + search box di sidebar | `DamkarTable.jsx` + `SearchBox.jsx` |

> Acid mengerjakan **semua panel sidebar** dan **halaman admin**. F5 dan F7 dari Acid harus bisa mengirim nilai ke komponen Trilen lewat props — koordinasikan interface-nya.
...
├── components/
│   ├── Sidebar/
│   │   ├── Sidebar.jsx           ← Wrapper sidebar kiri
│   │   ├── LayerControl.jsx      ← Checkbox toggle + slider radius
│   │   ├── StatsPanel.jsx        ← Kartu statistik blank spot
│   │   ├── RekomendasiPanel.jsx  ← List 5 kandidat rekomendasi
│   │   └── SearchBox.jsx         ← Search pos di sidebar
│   └── Admin/
│       ├── DamkarTable.jsx       ← Tabel CRUD + search
│       └── DamkarForm.jsx        ← Form tambah/edit pos
├── pages/
│   └── AdminPage.jsx             ← Halaman manajemen data
├── App.jsx                       ← Navbar + routing halaman
...
[ ] Buat StatsPanel.jsx
    → Fetch dari getStats()
    → Tampilkan 5 kartu: luas kota, pos, terlayani, blankspot, persen
[ ] Buat RekomendasiPanel.jsx
    → Fetch dari getRekomendasi()
    → List nama + luas + skor tiap kandidat
[ ] Buat LayerControl.jsx
    → Terima props: layers, onToggle, radius, onRadiusChange
    → 5 checkbox toggle layer
    → Slider radius 1000–10000
[ ] Buat SearchBox.jsx
    → Input text search nama pos
    → Saat ketik → filter marker di peta (koordinasikan props dengan Trilen)
[ ] Buat Sidebar.jsx
    → Wrapper: gabungkan StatsPanel + LayerControl + RekomendasiPanel + SearchBox
    → Lebar 300px, scroll vertikal

ADMIN
[ ] Buat DamkarTable.jsx
    → Fetch dari getDamkarList(search)
    → Input search filter real-time
    → Tabel: no_pos, nama_lokasi, lat, lng
    → Tombol Edit → callback ke AdminPage
    → Tombol Hapus → konfirmasi → deleteDamkar(id)
[ ] Buat DamkarForm.jsx
    → Props: editData (null = Create, ada data = Edit)
    → Form: nama_lokasi, no_pos, lat, lng, google_maps_li
    → Submit: createDamkar() atau updateDamkar()
[ ] Buat AdminPage.jsx
    → Tombol Tambah Pos → tampilkan DamkarForm mode Create
    → DamkarTable → klik Edit → tampilkan DamkarForm mode Edit
    → Setelah save/hapus → refresh tabel

NAVIGASI
[ ] Buat App.jsx
    → Navbar merah: "Peta" | "Manajemen Data"
    → State halaman: 'map' | 'admin'
    → Render MapPage (Trilen) atau AdminPage
[ ] Buat index.css (global style: reset, font, stat-card)
[ ] Test: navigasi peta ↔ admin berfungsi, CRUD dari UI berfungsi
[ ] Push ke branch → pull request
```

### Output yang Harus Diserahkan ke Tim

- ✅ Sidebar tampil lengkap dengan statistik + layer control + rekomendasi
- ✅ Halaman Admin: tabel tampil, search berfungsi, CRUD dari UI berfungsi
- ✅ Navbar navigasi Peta ↔ Manajemen Data berfungsi

---

## Koordinasi & Aturan Kerja Tim

### Urutan Kerja (Gantt Sederhana)

```
       H1          H2          H3          H4          H5          H6
DIMAS  [===SETUP===][=blankspot=][============TESTING & INTEGRASI=========]
RAFI              [=coverage==][=rekomendasi=][=jalan=][==TEST==]
ZAKKY             [====CRUD damkar.js====][=api.js=][==TEST==]
TRILEN                         [==Map components===========][==TEST==]
ACID                           [==Sidebar + Admin============][==TEST==]
```
...
| Kapan | Siapa | Apa yang Dikomunikasikan |
|-------|-------|--------------------------|
| Selesai H1 | Dimas → Semua | "db.js & app.js sudah push. Link repo: ..." |
| Selesai H2 | Zakky → Trilen & Acid | "api.js sudah push, nama fungsinya: ..." |
| Selesai H2 | Rafi → Semua | "Endpoint /coverage /rekomendasi /jalan sudah live" |
| Tengah H4 | Trilen → Acid | "MapPage.jsx interface props-nya begini: ..." |
| Selesai H5 | Semua → Dimas | "Bagianku sudah push, minta review" |
| H6 | Semua | Testing bareng, fix bug bersama |
...
```
1. Satu repo bersama → Dimas yang buat, undang semua
2. Setiap orang kerja di branch sendiri:
   - dimas/setup
   - rafi/backend-spatial
   - zakky/backend-crud
   - trilen/frontend-map
   - acid/frontend-ui
3. Selesai satu bagian → pull request → minta 1 orang review
4. Jangan langsung push ke branch main
5. Kalau ada konflik file → diskusi di grup dulu
```
...
| Anggota | Domain | File Utama | Prasyarat |
|---------|--------|-----------|-----------|
| **Dimas** | Backend Foundation + Analisis | `app.js`, `db.js`, `blankspot.js`, `stats.js`, `main.jsx` | — |
| **Rafi** | Backend Spatial API | `coverage.js`, `rekomendasi.js`, `jalan.js` | Dimas push `db.js` & `app.js` |
| **Zakky** | Backend CRUD + Bridge | `damkar.js` (full CRUD), `api.js` | Dimas push `db.js` & `app.js` |
| **Trilen** | Frontend Peta | Semua `Map/*.jsx`, `MapPage.jsx` | Dimas push `main.jsx` + Zakky push `api.js` |
| **Acid** | Frontend UI + Admin | Semua `Sidebar/*.jsx`, `Admin/*.jsx`, `AdminPage.jsx`, `App.jsx` | Dimas push `main.jsx` + Zakky push `api.js` |
